...
- **路由端点前缀** `/ai/v1/<provider>/*`（不是 `/v1/ai/*`，也不是单端点按 model 分发）：
  - domain-first 版本轴：AI 生态演进节奏显著快于业务 API（streaming 协议、tool use、function call 规范半年多变），让 AI 独立版本；流量 / 成本 / WAF 规则 / 限流按 `/ai/*` 前缀聚合
  - `<provider>` 入 URL：APISIX 3.16 的 `ai-proxy-multi` balancer 只做同服务多实例负载（HA），**不支持按 body.model 跨 provider 路由**；body-content 路由需要自写 Lua，与 ADR-0013 "APISIX 原生优先" 冲突。provider-in-URL 是 Cloudflare AI Gateway / Portkey / Helicone 的惯例
  - 客户端把 baseURL 设成 `https://<gateway>/ai/v1/openai`、`/ai/v1/claude`、`/ai/v1/deepseek`、`/ai/v1/qwen` 之一；OpenAI SDK 零改（provider 之间同 schema）
  - 未来真需要单端点 "智能分发"，再上独立 AI Gateway 服务（ADR-0014 的 Alt 1 路径），不动 APISIX 插件
- **provider 选择策略**：由 URL 前缀决定。客户端按需求选 `/ai/v1/openai`、`/ai/v1/claude`、`/ai/v1/deepseek`、`/ai/v1/qwen` 之一；请求 body 的 `model` 字段继续由客户端填（指定 provider 下具体哪个模型）。xinference 后续进入时加 `/ai/v1/local/*`
...
```
POST /ai/v1/<provider>/chat/completions
Authorization: Bearer <Authentik ID token>
Content-Type: application/json

{"model": "<provider 下的模型名>", "messages": [...]}
```

`<provider>` ∈ `openai | claude | deepseek | qwen`（后续可加 `local`）。响应形态与 OpenAI 一致（`ai-proxy` 把非 OpenAI 各家的响应转成 OpenAI schema）。业务代码一律用 OpenAI SDK，把 baseURL 配成 `http://<gateway>/ai/v1/<provider>` 即可。
