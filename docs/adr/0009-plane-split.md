...
- **容器数量**：当前 10+ 个（业务 ES 栈 3 + OTel ES 栈 3 + PG/Redis/MinIO/APISIX/etcd/Authentik 等），规划中图 DB/向量 DB/Kafka/Flink 等将推向 20+
...
| etcd | platform | config-store |
| apisix | platform | gateway |
| authentik（server + worker） | platform | iam |
...
### 类型嵌套（gateway/apisix/、iam/authentik/）
...
- Authentik 同 compose 直连 `postgres` 短名 + `depends_on` 恢复，无跨仓重试负担
...
`polaris-base-commons` + `polaris-base-data` 双仓。**问题**：跨 compose `depends_on` 失效（IAM ↔ PG）、`external: true` 网络耦合、两仓 CI/版本矩阵成本、与业界单仓模型背离。v4 当日回退。
...
### 按 role 分子目录（`components/gateway/apisix/`、`components/iam/authentik/`）
