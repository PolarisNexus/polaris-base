...
- **`polaris-base-commons` 创建 `polaris-net`**：顶层 compose 声明 `networks.polaris-net.name: polaris-net`，由 compose 自动创建（见 ADR-0009 仓库拆分）
- **`polaris-base-data` 加入 `polaris-net`**：声明 `external: true, name: polaris-net`
- **产品仓库加入 `polaris-net`**：同样 `external: true, name: polaris-net`
- 基座两仓每个服务注册 `base-<service>` 别名（如 `base-postgres`、`base-apisix`）
- 各 compose 项目内部通过 `default` 网络短名通信

**启动链**（commons → data → product）：commons 必须先启动以创建网络；data 与产品随后加入。commons 提供 `make up` 便利层依次启动 commons + data。
