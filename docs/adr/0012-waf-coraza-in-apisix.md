...
**客户端 IP 解析（2026-04-24 落地）**：`components/apisix/config.yaml` 已配 `nginx_config.http.real_ip_header: X-Forwarded-For` + `real_ip_recursive: on` + `real_ip_from` 信任列表（默认含 docker 桥 172.16.0.0/12、RFC1918 常见段）。生产部署在 LB / CDN 后需要把 `real_ip_from` 换成 LB 自身网段（Cloudflare 官方 IP 段 / ALB VPC CIDR 等），否则 nginx 要么完全不信 XFF（看到的永远是 LB IP），要么过度信任（任何客户端都能伪造 XFF 绕过封禁）。

### Turnstile（人机验证，2026-04-23 与 bouncer 合并落地）

没有新增 APISIX 自定义插件，Turnstile 集成做在既有 `crowdsec-bouncer` 侧车里，复用 forward-auth 通道：

- LAPI 的 `type=captcha` 决策进入 bouncer 的独立池（不与 `ban` 混淆）
- 挑战页由 bouncer 的 Go `html/template` 直接渲染（内嵌 Turnstile widget script）
- 挑战通过的 cookie：`polaris_captcha_pass = <exp_unix>.<hmac_sha256(exp_unix, secret)>`，HttpOnly，默认 1h TTL
- 验证回调 `/__polaris/captcha/verify` 由 APISIX 独立路由代理到 bouncer `/captcha/verify`；bouncer 自己 /check 看到 `X-Forwarded-Uri` 以 `/__polaris/` 开头直接放行，避免循环挑战

dev 默认用 Cloudflare 官方测试 key（`1x00000000000000000000AA` / `1x0000000000000000000000000000000AA`，siteverify 永远 success），生产必须覆盖 + 同时覆盖 `CAPTCHA_COOKIE_SECRET`（否则 cookie 可被伪造）。
